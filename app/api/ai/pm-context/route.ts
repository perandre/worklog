...
export async function GET(request: Request) {
  ...
  try {
    ...
    const [projects, activityTypes, allocations, existingRecords, timeLockDate] = await Promise.all([
      ...
      adapter.getAllocations(date).then((r) => { console.log(`[PM] getAllocations: ${r.length} items (${Date.now() - t0}ms)`); return r }),
      adapter.getExistingRecords(date).then((r) => { console.log(`[PM] getExistingRecords: ${r.length} items (${Date.now() - t0}ms)`); return r }),
